	...
	GridLineParticlesController() {
		for (int x = 0; x < ParticleSim.singleton.width; x += 1 / ParticleSim.singleton.res) {
			for (int y = 0; y < ParticleSim.singleton.height; y += 1 / ParticleSim.singleton.res) {
				...
				Particle particle = Particle.builder().currPos(new PVector(x, y)).pivotPos(new PVector(x, y)).charge(1)
						.build();
				pair.add(particle);
				particle = Particle.builder().currPos(new PVector(x, y)).pivotPos(new PVector(x, y)).charge(10).build();
				pair.add(particle);
